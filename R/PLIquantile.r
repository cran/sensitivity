...
PLIquantile = function(order,x,y,deltasvector,InputDistributions,type="MOY",samedelta=TRUE,percentage=TRUE,nboot=0,conf=0.95,bootsample=TRUE){
  ...
  ########################################	
    ...
    for (j in 1:length(lqid)){
      J[j,i]=lqid[j]
      if (percentage==FALSE) I[j,i]=transinverse(lqid[j],quantilehat,quantilehat)
      else I[j,i]=lqid[j]/quantilehat-1
      ...
      if (nboot > 0){
        ...
        if (percentage==FALSE){
          if (bootsample){
            ICIinf[j,i]=transinverse(qinf,qb,qb)
            ICIsup[j,i]=transinverse(qsup,qb,quantilehat)
          } else{
           ICIinf[j,i]=quantile(transinverse(lqidb[j,],quantilehatb,quantilehatb),(1-conf)/2)
           ICIsup[j,i]=quantile(transinverse(lqidb[j,],quantilehatb,quantilehatb),(1+conf)/2)
          }
        } else {
          if (bootsample){
            ...
            ICIinf[j,i]=quantile((lqidb[j,]/quantilehatb-1),(1-conf)/2)
            ICIsup[j,i]=quantile((lqidb[j,]/quantilehatb-1),(1+conf)/2)
          }
        }
      }
    }
