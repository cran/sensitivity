...
PLIquantile_multivar = function(order,x,y,inputs,deltasvector,InputDistributions,samedelta=TRUE,percentage=FALSE){
  ...
  I <- J <- matrix(0,ncol=nmbrededeltas,nrow=nmbrededeltas) 
  ...
  for (i in 1:nmbrededeltas){
    for (j in 1:nmbrededeltas){
      if (percentage==FALSE){
        if(lqid[i,j]>quantilehat){
          I[i,j]=lqid[i,j]/quantilehat-1
          J[i,j]=lqid[i,j]
        } else {
          I[i,j]=-quantilehat/lqid[i,j]+1
          J[i,j]=lqid[i,j]
        }
      } else{
        I[i,j]=lqid[i,j]/quantilehat-1
